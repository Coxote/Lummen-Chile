...
const REGION     = "na1";
const SCRIPT_SRC = "https://js.hsforms.net/forms/embed/developer/50799369.js";

declare global {
  interface Window {
    hbspt?: {
      forms: { create: (options: Record<string, unknown>) => void };
    };
  }
}
...
const IconSend = () => (
  <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
    <path d="M3 10L17 3L10 17L9 11L3 10Z" stroke="white" strokeWidth="1.6"
...
const IconRocket = () => (
  <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
    <path d="M10 2C10 2 14 4 14 10C14 13 12 15 10 16C8 15 6 13 6 10C6 4 10 2 10 2Z"
...
/* ── Skeleton loader ── */
const SkeletonBar = ({ width, height = 14, isDark }: { width: string; height?: number; isDark: boolean }) => (
  <div style={{
    width, height: `${height}px`, borderRadius: "6px",
...
/* ── Process step ── */
const ProcessStep = ({
  num, icon, title, desc, color, textMain, text, isLast, isDark,
}: {
  num: number; icon: React.ReactNode; title: string; desc: string;
  color: string; textMain: string; text: string; isLast: boolean; isDark: boolean;
}) => (
  <div style={{ display: "flex", gap: "16px", position: "relative" }}>
    ...
    <div style={{
      ...
      boxShadow: `0 0 16px ${color}44`, position: "relative", zIndex: 1,
    }}>
      {icon}
    </div>
    <div style={{ paddingBottom: isLast ? 0 : "20px" }}>
      <div style={{ marginBottom: "4px" }}>
        <span style={{
          ...
          fontSize: "11px", letterSpacing: "0.08em", color, opacity: 0.7,
        }}>PASO {num}</span>
      </div>
      <div style={{
        fontFamily: "'Urbanist', sans-serif", fontWeight: 700,
        fontSize: "15px", color: textMain, marginBottom: "4px",
      }}>{title}</div>
      <p style={{
        fontFamily: "'Urbanist', sans-serif", fontWeight: 400,
        fontSize: "13px", color: text, margin: 0, lineHeight: 1.6,
      }}>{desc}</p>
    </div>
  </div>
);
...
export const LumenHubSpotForm = () => {
  ...
  /* ── Design tokens — declared FIRST so useEffect can reference them ── */
  ...
  /* ── CSS injected via hbspt.forms.create({ css }) — declared before useEffect ── */
  const hubspotInlineCSS = `
    ...
    input::placeholder, textarea::placeholder {
      color: ${placeholderColor} !important; font-weight: 400 !important;
    }
    textarea { min-height: 100px !important; resize: vertical !important; }
    .hs-form-field { margin-bottom: 16px !important; }
    .hs-form-field .input { background: transparent !important; }
    .form-columns-2 {
      display: grid !important;
      grid-template-columns: 1fr 1fr !important;
      gap: 14px !important;
    }
    .form-columns-1 { margin-bottom: 0 !important; }
    ...
    .hs-error-msgs, .hs-error-msg {
      ...
      color: #FF5C5C !important; font-weight: 500 !important;
    }
    ...
    .hs-richtext p, .legal-consent-container p {
    ...
    .hs-richtext a, .legal-consent-container a {
      color: #8E00FF !important; text-decoration: underline !important;
    }
    .inputs-list { list-style: none !important; padding: 0 !important; margin: 0 !important; }
    .inputs-list label {
    ...
    .inputs-list input[type="checkbox"] {
      ...
      min-width: 16px !important; border-radius: 5px !important;
      margin-top: 1px !important; accent-color: #2D00FF !important;
    }
    .hs-form-boilerplate, .hs-form-boilerplate * { display: none !important; }
    * { box-sizing: border-box !important; }
  `;
  ...
  /* ── HubSpot traditional embed method ── */
  useEffect(() => {
    if (formRendered.current) return;
    
    const loadScript = () => {
      const existing = document.querySelector(`script[src="${SCRIPT_SRC}"]`);
      if (existing && window.hbspt) {
        // Script already loaded, render form
        renderForm();
        return;
      }
    ...
    const renderForm = () => {
      if (!window.hbspt || !formTargetRef.current) return;
  ...
  /* ── Page-level keyframes + HubSpot iframe styles ── */
  const pageStyles = `
    @keyframes shimmer {
      0%, 100% { opacity: 0.5; }
      50%       { opacity: 1; }
  ...
  return (
    <section
      ref={sectionRef}
      id="contacto-general"
      style={{ padding: `${isMobile ? "40px" : "80px"} ${px} ${isMobile ? "60px" : "100px"}`, position: "relative", overflow: "hidden" }}
    >
      ...
      <div style={{ maxWidth: "1280px", margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{
              fontFamily: "'Urbanist', sans-serif", fontWeight: 700,
              fontSize: "12px", color: badgeText, letterSpacing: "0.07em",
          ...
          <h2 style={{
            ...
            color: textMain, margin: "0 0 16px", lineHeight: 1.1,
          }}>
            Hablemos de{" "}
            <span style={{
        ...
        <div style={{
          ...
          <div style={{
            padding: "1.5px", borderRadius: "28px",
            background: isDark
            ...
            <div style={{ borderRadius: "27px", background: cardBg, overflow: "hidden" }}>
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <div>
                    ...
                    }}>LUMEN Motor Conversacional</div>
                  </div>
                </div>
                <div style={{
              ...
              <div style={{
                ...
                {/* Skeleton loader while HubSpot initialises */}
                {!formReady && (
                  <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                    <div style={{ display: "grid", gridTemplateColumns: isCompact ? "1fr" : "1fr 1fr", gap: "14px" }}>
                      {[...Array(4)].map((_, i) => (
                ...
                {/* HubSpot target div with unique ID */}
                <div
              ...
              <div style={{
                ...
                <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
                  <path d="M8 1L4 4V8C4 11.3 6.5 14 8 14.5C9.5 14 12 11.3 12 8V4L8 1Z"
                    stroke={isDark ? "rgba(255,255,255,0.3)" : "rgba(13,13,26,0.3)"} strokeWidth="1.2"
                    strokeLinecap="round" fill="none" />
                </svg>
                <span style={{
                  fontFamily: "'Urbanist', sans-serif", fontWeight: 500, fontSize: "11px",
